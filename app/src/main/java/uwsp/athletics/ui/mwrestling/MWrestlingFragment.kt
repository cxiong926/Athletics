...
import kotlinx.android.synthetic.main.fragment_mwrestling.*
import okhttp3.*
import uwsp.athletics.R
import java.io.IOException

class MWrestlingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_mwrestling, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {

        // Adds the viewpager to the fragment
        val mViewPager = view.findViewById(R.id.mWrestlingViewPager) as ViewPager
        mViewPager.adapter = MWrestlingPagerAdapter(childFragmentManager)

        //Adds the tab to the fragment
        mWrestlingTabLayout.setupWithViewPager(mViewPager)
        fetchMWrestlingScore()
        super.onViewCreated(view, savedInstanceState)

    }

    private fun fetchMWrestlingScore() {

        // Request URL for most recent game
        val url =
            "https://athletics.uwsp.edu/services/gameday.ashx?type=get-schedule-games&sport_id=19&take=1"
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                activity?.runOnUiThread(java.lang.Runnable {
                    ...
                    val date: TextView? = view?.findViewById(R.id.mWrestlingDate)
...
//                    time?.text = scoreReturn[0].time
                    ...
                    val awayTeam: TextView? = view?.findViewById(R.id.mWrestlingOppTeam)
                    val homeTeam: TextView? = view?.findViewById(R.id.mWrestlingUWSPTeam)
                    ...
                    val awayTeamPic: ImageView = view!!.findViewById(R.id.mWrestlingOppPic)
                    val homeTeamPic: ImageView = view!!.findViewById(R.id.mWrestlingUWSPPic)

                    // Loads the away team's image from the returned URL
                    Glide.with(this@MWrestlingFragment)
                    ...
                    val uwspIndicator: TextView? = view?.findViewById(R.id.mWrestlingUWSPIndicator)
                    val oppIndicator: TextView? = view?.findViewById(R.id.mWrestlingOppIndicator)
