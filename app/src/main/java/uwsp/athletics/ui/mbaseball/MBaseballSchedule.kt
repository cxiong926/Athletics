 ...
 */
class MBaseballSchedule : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_mbaseball_schedule, container, false)
    }


    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val lin: LinearLayout = view.findViewById(R.id.mBaseSchedLinear)
        fetchMBaseSchedule(lin)
        super.onViewCreated(view, savedInstanceState)
    }

    private fun fetchMBaseSchedule(layout: LinearLayout) {

        // Request URL for  the men's baseball schedule
        val url =
            "https://athletics.uwsp.edu/services/schedule_xml_2.aspx?format=json&path=baseball&take=100"
...
//                        gameDate.setTextColor(Color.parseColor("#000000"))
                        ...
                        if (sidearmReturn.schedule[i].results.isNotEmpty()) {
                            ...
                            val boxscore: TextView = TextView(currentContext)
                            val recap: TextView = TextView(currentContext)
                            gameCompleted = true

                            // Sets the styles of the textview
                            boxscore.textSize = 14.0f
                            boxscore.setTextColor(Color.parseColor("#1a0dab"))
                            boxscore.setPadding(0, 0, 32, 20)
                            boxscore.append("\u25BA View Box Score")

                            // sets the onclicklistener for the boxscore
                            boxscore.setOnClickListener {
                                val openURL = Intent(Intent.ACTION_VIEW)
                                openURL.data =
                                    Uri.parse(sidearmReturn.schedule[i].links.boxscore.url)
                            ...
                            linksLinearLayout.addView(boxscore)
                        ...
                        Glide.with(this@MBaseballSchedule)
...
//class GetRoster(val items:List<Roster>)
//class Roster(val roster: String)
